...
class _PaymentPendingPageState extends State<PaymentPendingPage> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    } else if (widget.order.paymentMethod == 'global_institute_pay') {
      WidgetsBinding.instance.addPostFrameCallback((_) => _launchGlobalInstitutePay());
    }

    final pending = PaymentDeeplinkService().consumePendingCallback();
    if (pending != null && pending.isSuccess) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _onPaymentSuccess());
    }

    _callbackSub = PaymentDeeplinkService().onCallback.listen((data) {
      if (!mounted) return;
      if (data.isSuccess) {
        _onPaymentSuccess();
      } else {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && (_gopayLaunched || _payLaunched)) {
  ...
  Future<void> _launchGlobalInstitutePay() async {
    ...
    final deeplinkUrl = PaymentDeeplinkService.buildDeeplinkUrl(
    ...
    Navigator.pushAndRemoveUntil(
      ...
      MaterialPageRoute(
        builder: (context) => OrderSuccessPage(
          order: context.read<OrderProvider>().lastOrder ?? widget.order,
        ),
      ),
      (route) => route.isFirst,
    );
  ...
  void _showCancelConfirmation() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Pesanan tetap tersimpan. Kamu bisa bayar nanti di halaman "Pesanan Saya".'),
        actions: [
          TextButton(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                Text('Total Pembayaran', style: TextStyle(fontSize: 14, color: onSurface)),
                Text(
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                _StepItem(
                  number: '1',
                  text: gopayLaunched ? 'Aplikasi GoPay sudah dibuka' : 'Kamu akan diarahkan ke aplikasi GoPay',
                  done: gopayLaunched,
                ),
    ...
    return Row(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            color: done ? Colors.green : Theme.of(context).colorScheme.primary.withValues(alpha: 0.12),
          ),
          child: Center(
            child: done
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                _StepItem(
                  number: '1',
                  text: payLaunched ? 'Aplikasi Dompet Kampus sudah dibuka' : 'Kamu akan diarahkan ke Dompet Kampus',
                  done: payLaunched,
                ),
                const SizedBox(height: 14),
                _StepItem(
                  number: '2',
                  text: 'Konfirmasi pembayaran dengan PIN & 2FA',
                  done: false,
                ),
                const SizedBox(height: 14),
                const _StepItem(
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              icon: const Icon(Icons.open_in_new),
              label: Text(
                payLaunched ? 'Buka Kembali Dompet Kampus' : 'Buka Dompet Kampus',
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
              ),
              onPressed: onOpenApp,
            ),
