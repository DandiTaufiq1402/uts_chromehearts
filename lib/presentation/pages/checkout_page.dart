...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  Future<void> _placeOrder(BuildContext context) async {
    ...
    if (success) {
      // Tidak perlu panggil clearCart karena sudah dilakukan di repository
      // Tapi update local state cart provider
      cartProv.clearCart();

      if (!context.mounted) return;
      final order = orderProv.lastOrder!;
      final needsPaymentFlow = order.paymentMethod == 'virtual_account' || order.paymentMethod == 'global_institute_pay';
            ...
            children: [
              ...
              ..._paymentOptions.map(
                (option) => _PaymentOptionCard(
                  ...
                  onSelect: () => setState(() => _selectedPaymentMethod = option.value),
                ),
              ),
